      ...
      <div class="w-2/3 flex flex-col">
        <div class="flex flex-row p-3 rounded-t-lg bg-blue-200 border-b border-gray-400">
          <div class="w-1/2 mr-2">
            <input type="search" class="w-full px-2 rounded-lg"  style="float:left;" v-model="filter" v-on:keyup="renderList()" />
            <i class="fas fa-search text-gray-200" style="float:right;margin-top:-20px;margin-right:9px;"></i>
          </div>
          <div class='w-1/2 flex flex-row'>
            <div class='w-1/2 text-right mr-2'>Group By</div>
            <div class='w-1/2'>
              <select @change="renderList()" v-model="selectedAttribute" class="text-sm w-48">
                <option v-for="grp in groupByOptions" class='text-xl'
                  v-bind:key="grp.value"
                  v-bind:value="grp.value">{{grp.text}}</option>
              </select>
            </div>
          </div>
        </div>
        <div class="bg-gray-100 pt-4 rounded-b-lg border border-gray-200 " style="min-height: 580px;z-index:0">
